...
%% Enlarged walls map definition
% maximum Turtlebot dimension is 306 (mm)
res = 0.05; %[m/pix]
mENL = (0.5*310)*1e-3;            %[m]

eta = 5;                          %[-], safety coefficient

enl = floor(eta*mENL/res);        %[pix] needed augmentation
...
[conNodes, costNodes] = conNodescalc(BWe, start);
...
[con_nodes, ~] = conNodescalc(BWe, start);
...
while any(nodelist(:, 1) == 1) && cur_node ~= goal
    ...
    [~, cost_con_nodes] = conNodescalc(BWe, cur_node);

    while i_con > 0 % avail & not visited

        ny = fix(con_nodes(i_con)/size(BWe, 1)) +  1*(mod(con_nodes(i_con), size(BWe, 1)) ~= 0);
        nx = con_nodes(i_con) - (ny - 1)*size(BWe, 1);
        ...
        elseif ( dist(con_nodes(i_con)) > dist(cur_node) + sqrt(2) +  CostToGo) && DiagCond
...
%     plot_runtime(cur_node, BW)
    ...
    nodelist(cur_node, 1) = 0; % visited
    [con_nodes, ~] = conNodescalc(BWe, cur_node);
    i_con = length(con_nodes);
