...
scanBag = select(bagselect, 'Time', [bagselect.StartTime bagselect.EndTime], 'Topic', '/scan');
...
scanTime = extractTime(scanStructs, length(scanStructs));
...
scatter(WList(:,1), WList(:,2), '*k', 'LineWidth', 8)

for ii = 1:length(WList)
    hold on
    a = WList(ii,1);
    b = a + 1*cos(WList(ii,3));
    c = WList(ii,2);
    d = c + 1*sin(WList(ii,3));
...
hold on
scatter(RBList(:, 1), RBList(:, 2), bSize, "red", 'filled')
hold on
scatter(GBList(:, 1), GBList(:, 2), bSize, "green", 'filled')
