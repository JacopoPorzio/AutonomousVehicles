...
while any(nodelist(:, 1) == 1) && cur_node ~= goal
    ...
    while i_con > 0 % avail & not visited
        ...
        switch AssignmentCase
            ...
            case 'Diagonal'
                    a1 = ( dist(con_nodes(i_con)) > dist(cur_node) + 1 );
                    a2 = ( dist(con_nodes(i_con)) > dist(cur_node) + sqrt(2));
                    if (a1 && ~a2) && G(cur_node, con_nodes(i_con)) == 1
                        dist(con_nodes(i_con)) = dist(cur_node) + 1;
                        prec(con_nodes(i_con)) = cur_node;
                    elseif a2 && G(cur_node, con_nodes(i_con)) == sqrt(2) % indecision
            ...
            case 'DiagonalDijkstra+A*'
                    ...
                    nx = con_nodes(i_con) - (ny - 1)*size(BW, 1);
                    CostToGo = sqrt((nx - goal_pos(1))^2 + (ny - goal_pos(2))^2);
                    a1 = ( dist(con_nodes(i_con)) > dist(cur_node) + 1 + CostToGo);
                    a2 = ( dist(con_nodes(i_con)) > dist(cur_node) + sqrt(2) + CostToGo);
%                     if a1 && G(cur_node, con_nodes(i_con)) == 1
...
%                     end
                    ...
                    if a2 && G(cur_node, con_nodes(i_con)) == sqrt(2)
                        dist(con_nodes(i_con)) = dist(cur_node) + sqrt(2) + CostToGo;
                        prec(con_nodes(i_con)) = cur_node;
                    elseif a1 && G(cur_node, con_nodes(i_con)) == 1
                        dist(con_nodes(i_con)) = dist(cur_node) + 1 + CostToGo;
                        prec(con_nodes(i_con)) = cur_node;                        
                    end
        end
