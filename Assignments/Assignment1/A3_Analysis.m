...
% clBag = select(bagselectIN, 'Time', [bagselectIN.StartTime bagselectIN.EndTime], 'Topic', '/clock');

odomStructs = readMessages(odomBag, 'DataFormat','struct');
odomTime = odomBag.MessageList.Time;

% clStructs = readMessages(clBag, 'DataFormat','struct');
% clTime = clBag.MessageList.Time;

% DT = mean(diff(odomTime));
% 
% odomTime = (odomTime(1):DT:odomTime(end))';
% 
