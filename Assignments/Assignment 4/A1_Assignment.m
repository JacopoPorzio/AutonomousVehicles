...
AssignmentCase = 'DiagonalDijkstra+A*';
    ...
    for jj = 1:size(BW, 2)
        if BW(ii, jj) == 1 
            ...
            if strcmp(AssignmentCase, 'Diagonal') || strcmp(AssignmentCase, 'DiagonalDijkstra+A*')
                ...
                % SW /|\ SE
                %
                SWCon = (ii + 1 > 0 && jj - 1 > 0) && (ii + 1 <= size(BW, 1) && jj - 1 <= size(BW, 2)) && (BW(ii + 1, jj) == 1 && BW (ii, jj - 1) == 1);
                NWCon = (ii - 1 > 0 && jj - 1 > 0) && (ii - 1 <= size(BW, 1) && jj - 1 <= size(BW, 2)) && (BW(ii - 1, jj) == 1 && BW (ii, jj - 1) == 1);
                SECon = (ii + 1 > 0 && jj + 1 > 0) && (ii + 1 <= size(BW, 1) && jj + 1 <= size(BW, 2)) && (BW(ii + 1, jj) == 1 && BW (ii, jj + 1) == 1);
                NECon = (ii - 1 > 0 && jj + 1 > 0) && (ii - 1 <= size(BW, 1) && jj + 1 <= size(BW, 2)) && (BW(ii - 1, jj) == 1 && BW (ii, jj + 1) == 1);
        ...
        switch AssignmentCase
            ...
            case 'Diagonal'
                    if ( dist(con_nodes(i_con)) > dist(cur_node) + 1 ) && G(cur_node, con_nodes(i_con)) == 1
                        ...
                        prec(con_nodes(i_con)) = cur_node;
                    elseif ( dist(con_nodes(i_con)) > dist(cur_node) + sqrt(2) + 1*0 ) && G(cur_node, con_nodes(i_con)) == sqrt(2) % indecision
            ...
            case 'DiagonalDijkstra+A*'
                    ...
                    if ( dist(con_nodes(i_con)) > dist(cur_node) + 1 + CostToGo) && G(cur_node, con_nodes(i_con)) == 1
                        ...
                        prec(con_nodes(i_con)) = cur_node;
                    elseif ( dist(con_nodes(i_con)) > dist(cur_node) + sqrt(2) + 1*0 + CostToGo) && G(cur_node, con_nodes(i_con)) == sqrt(2) % indecision
